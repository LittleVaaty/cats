...
export class EntitySheetHelper {
  static getAttributeData(data) {
    ...
    Object.keys(data.system.attributes)
        ...
        for (let i = 0; i < groups.length; i++) {
          if (groups[i].value === val) {
            ui.notifications.error(
              game.i18n.localize("CATS.NotifyAttrDuplicate") + ` (${val})`
            );
            el.value = oldVal;
            attrError = true;
            break;
  ...
  static validateGroup(groupName, document) {
    ...
    if (groups.includes(groupName)) {
      ui.notifications.error(
        game.i18n.localize("CATS.NotifyGroupDuplicate") + ` (${groupName})`
      );
      return false;
    }
    ...
    if (attributes.includes(groupName)) {
      ui.notifications.error(
        game.i18n.localize("CATS.NotifyGroupAttrDuplicate") + ` (${groupName})`
      );
      return false;
    }
    ...
    if (["attr", "attributes"].includes(groupName)) {
      ui.notifications.error(
        game.i18n.format("CATS.NotifyGroupReserved", { key: groupName })
      );
      return false;
    }
    ...
    if (groupName.match(/[\s|\.]/i)) {
      ui.notifications.error(
        game.i18n.localize("CATS.NotifyGroupAlphanumeric")
      );
      return false;
    }
    return true;
