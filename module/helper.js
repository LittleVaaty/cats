...
export class EntitySheetHelper {
      ...
      if ( el.classList.contains("attribute-key") ) {
        ...
        for ( let i = 0; i < groups.length; i++ ) {
          if (groups[i].value === val) {
            ui.notifications.error(game.i18n.localize("CATS.NotifyAttrDuplicate") + ` (${val})`);
            el.value = oldVal;
            attrError = true;
            break;
          }
  ...
  static validateGroup(groupName, document) {
    ...
    if ( groups.includes(groupName) ) {
      ui.notifications.error(game.i18n.localize("CATS.NotifyGroupDuplicate") + ` (${groupName})`);
      return false;
    }

    // Check for group keys that match attribute keys.
    if ( attributes.includes(groupName) ) {
      ui.notifications.error(game.i18n.localize("CATS.NotifyGroupAttrDuplicate") + ` (${groupName})`);
      return false;
    }

    // Check for reserved group names.
    if ( ["attr", "attributes"].includes(groupName) ) {
      ui.notifications.error(game.i18n.format("CATS.NotifyGroupReserved", {key: groupName}));
      return false;
    }

    // Check for whitespace or periods.
    if ( groupName.match(/[\s|\.]/i) ) {
      ui.notifications.error(game.i18n.localize("CATS.NotifyGroupAlphanumeric"));
      return false;
    }
    return true;
  }
  ...
  static async deleteAttributeGroup(event, app) {
    ...
    new Dialog({
      title: game.i18n.localize("CATS.DeleteGroup"),
      content: `${game.i18n.localize("CATS.DeleteGroupContent")} <strong>${group.val()}</strong>`,
      buttons: {
        confirm: {
  ...
  static async createDialog(data={}, options={}) {
    ...
    const useEntity = game.cats.useEntity;
    ...
    return Dialog.prompt({
      ...
      callback: html => {
        ...
        if ( template ) {
          ...
          delete createData.flags.cats.isTemplate;
        }
